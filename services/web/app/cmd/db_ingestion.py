...
def extract_operator(i, offset):
    ...
    if operator_not_null(i, offset):
        nik = i[1] if offset == 0 else None
        return models.Operator(
            **{"id": operator_id, "name": i[0 + offset].title(), "nik": nik}
        )
    return None
...
def import_data(filename, model, get_id, extract_data, data_not_null, offset):
    ...
    with open(filename, newline="") as csvfile:
        ...
        data_to_input = {
            id: extract_data(i, offset)
            for i in csvreader[1:]
            if (
                data_not_null(i, offset)
                and (id := get_id(i, offset)) not in all_data_id
            )
        }
...
def import_to_db(filename):
    ...
    import_mesin("data/db/db_mesin.csv")
    import_operator("data/db/db_operator.csv")
    session.commit()
...
if __name__ == "__main__":
    import_to_db("data/db/data_all.csv")
