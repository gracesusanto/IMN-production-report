...
def get_mesin_status(session=Sessioner):
    ...
    mesin_status = session.query(models.MesinStatus).\
        ...
        all()
    return { "details": sorted(mesin_status + mesin_status_idle, key=lambda d: d['Mesin']) }
    # return { "details": 
    # sorted([schema.MesinStatusAll.from_orm(mesin).dict(exclude_none=True) \
    #     for mesin in mesin_status + mesin_status_idle], key=lambda d: d['Mesin']) 
    #     }
...
def validate_input(tooling_id, mesin_id, operator_id, session=Sessioner):
    if session.query(models.Mesin).filter(models.Mesin.id == mesin_id).first() is None or \
        session.query(models.Tooling).filter(models.Tooling.id == tooling_id).first() is None or \
        session.query(models.Operator).filter(models.Operator.id == operator_id).first() is None:
...
def check_operator_status(request: schema.Detail, session=Sessioner):
    isSuccess, errorMessage = validate_input(tooling_id=request.tooling_id, 
...
def post_activity(activity: schema.Activity, session=Sessioner):
    isSuccess, errorMessage = validate_input(tooling_id=activity.tooling_id, 
