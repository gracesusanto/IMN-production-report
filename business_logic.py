...
def start_activity(tooling_id, mesin_id, operator_id, session):
    ...
    last_downtime = models.LastDowntimeMesin(
        ...
        downtime_category = mesin_status.last_stop.downtime_category
    )
    session.add(last_downtime)
    session.commit()

    # Update downtime for prev operator
    if (mesin_status.last_stop.downtime_category[:2].upper() != "NP"):
...
def first_stop_activity(tooling_id, mesin_id, operator_id, output, downtime_category, session):
    ...
    if mesin_status.status == models.Status.IDLE:
        raise HTTPException(
    ...
    mesin_status.status = models.Status.IDLE
    mesin_status.last_stop = stop_entity
    mesin_status.last_tooling_id = tooling_id
    mesin_status.last_operator_id = operator_id
    session.commit()

def continue_stop_activity(tooling_id, mesin_id, operator_id, downtime_category, session):
    # Insert to Stop Table
    stop_entity = models.Stop(
        tooling_id = tooling_id,
        mesin_id = mesin_id,
